...
def init_db():
  ...
  with sqlite3.connect(DB_NAME) as conn:
    conn.execute("""
        ...
        """)
    # Initialize challenge balance to $100 if not set
    conn.execute(
        "INSERT OR IGNORE INTO challenge (id, current_balance) VALUES (1,"
        " 100.0);"
    )
...
def add_trade():
    ...
    label = data.get("label")  # e.g., Buy / Sell
    ...
    with sqlite3.connect(DB_NAME) as conn:
        ...
        # Update Challenge Balance ($100 to $1000 goal)
        conn.execute(
            """
                UPDATE challenge 
